...
python add_prefix.py --input_dic $DIR/dic --output_dic $DIR/dic_prefix -prefix $prefix -input_ord $language2 -output_ord $DIR/$source.ord.prefix_nocut -input_pre $DIR/pre.$source.clean -output_pre $DIR/$source.pre.prefix_nocut

echo 'turn tokens into index'

python from_word_to_num.py -d $DIR/dic_prefix -pre $DIR/$source.pre.prefix_nocut -ord $DIR/$source.ord.prefix_nocut -ord_num $DIR/$source.ord.prefix.num_nocut -pre_num $DIR/$source.pre.prefix.num_nocut
